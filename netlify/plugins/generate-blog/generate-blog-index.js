  ...
  <div class="con hdr-inner">
    <a href="/index.html" class="logo" aria-label="UK Rugby Club Directory">
      <svg viewBox="0 0 220 48" xmlns="http://www.w3.org/2000/svg" height="48" role="img" aria-label="UK Rugby Club Directory" style="display:block">
        ...
        <rect x="35" y="7" width="2.5" height="34" rx="1.25" fill="#7c5cfc"/>
        <text x="46" y="26" font-family="Arial,Helvetica,sans-serif" font-size="17" font-weight="700" fill="#0f0c1d" letter-spacing="-0.3">Rugby Club</text>
        <text x="46" y="40" font-family="Arial,Helvetica,sans-serif" font-size="9.5" fill="#8b87a8" letter-spacing="3.5">DIRECTORY</text>
      </svg>
    </a>
      ...
      <div class="ft-brand">
        <a href="/index.html" class="ft-logo">
          <svg viewBox="0 0 200 40" xmlns="http://www.w3.org/2000/svg" height="40" role="img" aria-label="UK Rugby Club Directory" style="display:block">
            ...
            <text x="39" y="33" font-family="Arial,Helvetica,sans-serif" font-size="8" fill="rgba(255,255,255,0.72)" letter-spacing="3">DIRECTORY</text>
          </svg>
        </a>
        <p class="ft-strapline" style="color:rgba(255,255,255,.78)">The UK's most comprehensive free rugby club directory. 620 clubs each with their own full profile page, unique SEO and verified contact details across England, Scotland, Wales and Northern Ireland.</p>
...
<head>
  ...
  <style>
    ...
    .blog-section-eyebrow{font-size:.68rem;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:#7c5cfc;margin:0 0 .3rem}
    ...
    .blog-card-cat{position:absolute;top:.85rem;left:.85rem;font-size:.65rem;font-weight:700;letter-spacing:.07em;text-transform:uppercase;background:#7c5cfc;color:#fff;padding:.22rem .65rem;border-radius:100px}
    ...
    .blog-card-title{font-family:var(--fd,'Inter',sans-serif);font-size:1rem;font-weight:700;text-transform:none;letter-spacing:-.01em;color:var(--navy,#0a1628);line-height:1.25;margin:0}
    .blog-card:hover .blog-card-title{color:#7c5cfc}
    .blog-card-excerpt{font-size:.86rem;color:#475569;line-height:1.6;margin:0;flex:1}
    .blog-card-cta{margin-top:auto;padding-top:.75rem;border-top:1px solid #f0ecff;font-size:.78rem;font-weight:700;color:#7c5cfc;display:flex;align-items:center;gap:.3rem;transition:gap .15s}
    ...
    /* Req 2: hero image */
    .article-hero-image-wrap{max-width:1160px;margin:0 auto;padding:0 1.5rem}
    .article-hero-image-wrap img{width:100%;max-height:384px;object-fit:cover;border-radius:12px;display:block;box-shadow:0 4px 20px rgba(15,12,29,.12);margin-top:1.5rem}
    ...
    .article-body ul li::before{content:"";position:absolute;left:0;top:.65em;width:6px;height:6px;border-radius:50%;background:#7c5cfc;flex-shrink:0}
    .article-body ol li{margin-bottom:.55rem;padding-left:.25rem}
    .article-body ol li::marker{color:#7c5cfc;font-weight:700}
    ...
    .article-body a{color:#7c5cfc;text-decoration:underline;text-underline-offset:2px}
    .article-body a:hover{color:#6244e8}
    .article-body blockquote{border-left:4px solid #7c5cfc;margin:2rem 0;padding:1rem 1.5rem;background:linear-gradient(135deg,#fafafa 0%,#f1f5f9 100%);border-radius:0 8px 8px 0;color:#475569;font-style:italic;font-size:1.05rem}
    .article-body blockquote p{margin:0}
    .article-body code{background:#f0ecff;padding:.15rem .45rem;border-radius:4px;font-family:'Courier New',Courier,monospace;font-size:.86em;color:#6244e8;border:1px solid #e2e8f0}
    ...
    .sidebar-card{background:#ffffff;border:1px solid #e4e1f5;border-radius:12px;padding:1.35rem;box-shadow:0 2px 8px rgba(15,12,29,.06)}
    .sidebar-card h3{font-family:var(--fd,'Inter',sans-serif);font-size:.9rem;font-weight:700;color:#0f0c1d;margin:0 0 .5rem;text-transform:none;letter-spacing:-.01em}
    .sidebar-card p{font-size:.85rem;color:#64748b;line-height:1.55;margin:0 0 1rem}
    .sidebar-cta{display:inline-block;padding:.6rem 1.1rem;background:#7c5cfc;color:#fff;border-radius:100px;font-size:.85rem;font-weight:600;text-decoration:none;transition:background .15s}
    .sidebar-cta:hover{background:#6244e8}
    .back-link{display:inline-flex;align-items:center;gap:.35rem;color:#7c5cfc;font-size:.85rem;font-weight:600;text-decoration:none;margin-bottom:2rem}
    ...
    .toc-list li a:hover{color:#7c5cfc;background:#f0ecff;border-left-color:#7c5cfc}
    .toc-list li a.toc-active{color:#7c5cfc;background:#f0ecff;border-left-color:#7c5cfc;font-weight:600}
    /* ── E-E-A-T trust bar ───────────────────────────────────────────────── */
    ...
    /* ── Social share ────────────────────────────────────────────────────── */
    .share-section{margin:2.5rem 0 0;padding:1.5rem;background:#7c5cfc;border-radius:14px;display:flex;align-items:center;flex-wrap:wrap;gap:1rem}
...
const RUGBY_IMAGES = [
  ...
  { src: "/contact page.jpg",             alt: "Rugby supporters and community at a UK club" },
  { src: "/rugby-joints.jpg",             alt: "Rugby club facilities and community" },
];
  ...
  const cards = articles.length === 0
    ...
    : articles.map(a => `
      <a href="/blog/${a.slug}/" class="blog-card">
        <div class="blog-card-img" style="padding:0;background:#e8e4f8">
          <img src="${a.image || '/imgs/rugby-union-hero.jpg'}" alt="${a.title}" style="width:100%;height:190px;object-fit:cover;display:block" loading="lazy">
...
function buildArticlePage(slug, fm, bodyHtml) {
  ...
  const bodyContent = `
    ...
    <div class="clubs-nearby-widget">
      <h3>Find a Rugby Club Near You</h3>
      <p>Search over 620 clubs across England, Scotland, Wales and Northern Ireland — every club with its own full profile page.</p>
      <a href="/clubs.html" class="btn btn-outline-wh btn-lg">Browse All Clubs</a>
    </div>
        ${authorBioHtml}
        ${shareHtml}
      </div>
